...
def run_report(report_id: str) -> dict:
    ...
    try:
        readiness = require_report_ready(report["scope_id"])
        dashboard = readiness["dashboard"]
        ...
        row_count = len(indicators) + len(dianping) + len(xiaohongshu)
        metadata = {
            "scope": readiness["scope"],
            "stat_date": readiness["stat_date"],
            "crawl_date": readiness["crawl_date"],
            ...
            "xiaohongshu_age_hours": readiness["xiaohongshu_age_hours"],
            "sheets": ["报告摘要", "指标快照", "点评明细", "小红书明细"] if report["file_format"] == "xlsx" else ["指标快照"],
        }
